...
class TimestepBlock(nn.Module):
    ...
    """
    @abstractmethod
    def forward(self, x, time_emb, cond_emb, inivp, ref, well, well_loc_emb):
        pass
    ...
    def forward(self, x, time_emb, cond_emb, inivp, ref, well, well_loc_emb):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, time_emb, cond_emb, inivp, ref, well, well_loc_emb)
            else:
                x = layer(x)
        return x
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        if inivp_channels is not None:
            self.inivp_emb_conv = nn.Sequential(
                normalization(inivp_channels), SiLU(),
                conv_nd(dims, inivp_channels, self.out_channels, 3, padding=1),
            )
    ...
    def forward(self, x, time_emb, cond_emb, inivp_emb=None, ref_emb=None, well_emb=None, well_loc_emb=None):
        # Optionally use checkpointing to save memory
        return checkpoint(self._forward,
                          (x, time_emb, cond_emb, inivp_emb, ref_emb, well_emb, well_loc_emb),
                          self.parameters(), self.use_checkpoint)

    def _forward(self, x, time_emb, cond_emb, inivp_emb, ref_emb, well_emb, well_loc_emb):
        ...
        # Fuse shallow/structural/well/background velocity conditions
        B, C, H, W = out.shape
        cond_ds = F.adaptive_avg_pool2d(cond_emb, (H, W))
        cond_ds = self.condtop_emb_conv(cond_ds)
        # Add background velocity info if present
        if inivp_emb is not None:
            bg_ds = F.adaptive_avg_pool2d(inivp_emb, (H, W))
            bg_ds = self.inivp_emb_conv(bg_ds)
            cond_ds = cond_ds + bg_ds
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        self.inivp_embed = nn.Sequential(
    ...
    def forward(self, inp, cond_top, inivp, ref, well, well_loc, timesteps, y=None):
        """
        ...
        # 4) Well & structural & background velocity embeddings if provided
        if inivp is not None:
            inivp = self.check_image_size(inivp)
            inivp = self.inivp_embed(inivp)
        if well is not None:
            well = self.check_image_size(well)
        ...
        for module in self.downs:
            x = module(x, time_emb, cond_top, inivp, ref, well, well_loc)
            skips.append(x)
        # 7) Bottleneck
        x = self.middle(x, time_emb, cond_top, inivp, ref, well, well_loc)
        # 8) Decoding
        for module in self.ups:
            cat_in = th.cat([x, skips.pop()], dim=1)
            x = module(cat_in, time_emb, cond_top, inivp, ref, well, well_loc)
