"""
Sampling script for Part I of the depth-progressive velocity model building manuscript.
...
Structural constraints are provided via a reflectivity model derived from the
true velocity, which serves as a proxy for migration-based structural images
...
toggling well-log constraints (use_well) and reflectivity constraints (use_ref),
and optionally applies gradient-based well guidance during reverse diffusion.
Results (ensemble mean, uncertainty, and individual samples) are saved as .mat
...
def main(use_well, use_ref):
    ...
    for idx, md in enumerate(model_list):
        ...
        dict_ = sio.loadmat(f'../dataset/test/{md}_vel.mat')
        ...
        accumulated_mean[:args.depth_size, :]   += gaussian1d_z * denormalizer_vel(vp_top[0].squeeze())
        ...
        for iz in indices_z:
            ...
            accumulated_mean[iz:iz + args.depth_size]   += gaussian1d_z * denormalizer_vel(sample.squeeze()).mean(dim=0)
            accumulated_std[iz:iz + args.depth_size]    += gaussian1d_z * denormalizer_vel(sample.squeeze()).std(dim=0)
        ...
        final_sample = denormalizer_vel(accumulated_sample / accumulated_weight_batch)  # (B, nz, nx)

        # L1 accuracy of the ensemble mean vs. the ground-truth velocity.
        with th.no_grad():
            accs = criterion(final_pred_mean, denormalizer_vel(vp[0].squeeze()))
